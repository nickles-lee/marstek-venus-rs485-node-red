...
### More an insights tool than a debug tool
It shows *known issues*, but is not a full debug tool. Some unforeseen errors still require inspection via Node-RED. Also the Node-RED logs can be useful, e.g. `http://homeassistant.local:8123/hassio/addon/******_nodered/logs`
...
#### Insights shows `Timeout` at `Call "Link in" node`
- Most likely: the system has become too slow. Check [the Flow Trace](#system-seems-slow-or-unresponsive)
- Using a Raspberry Pi? [Check if it has sufficient power](#raspberry-pi-slows-down-due-to-lack-of-power)
- Otherwise check Node-RED and review the strategy flows. Does it show red dots or errors?
...
- Verify grid power sensor is updating correctly (check timestamps, the interval should be regular steps of +/- 1 sec.)

## Other Common Issues

#### The controller barely responds and (dis)charges only with a few Watts
- Check if the P1 input is in Watt and not in kW.
